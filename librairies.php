      ...
      <div id="content-wrapper">
        <div class="container-fluid">
          <ol class="breadcrumb bg-dark">
            ...
            <li class="breadcrumb-item active">Liste des Librairies</li>
          </ol>
	<div class="row">
		<div class="col-xl-12 col-sm-6 mb-3">
			<div class="col-lg-12 header"><h1 class="section-heading text-uppercase" align="center"><i class="fas fa-users"></i> Nos librairies partenaires</h1>
			</div>
			<div class="col-xl-12 col-sm-6 mb-3 content">
			<?php 
				$librairieParPage=5; //Nous allons afficher 5 librairies par page.
//Une connexion SQL doit être ouverte avant cette ligne...
$retour_total=mysqli_query($db,'SELECT COUNT(*) AS total FROM librairie'); //Nous récupérons le contenu de la requête dans $retour_total
$donnees_total=mysqli_fetch_assoc($retour_total); //On range retour sous la forme d'un tableau.
$total=$donnees_total['total']; //On récupère le total pour le placer dans la variable $total.
 
//Nous allons maintenant compter le nombre de pages.
$nombreDePages=ceil($total/$librairieParPage);
...
$premiereEntree=($pageActuelle-1)*$librairieParPage; // On calcul la première entrée à lire
 
// La requête sql pour récupérer les librairies de la page actuelle.
$retourLibrairie=mysqli_query($db,'SELECT * FROM librairie ORDER BY id_lib LIMIT '.$premiereEntree.', '.$librairieParPage.'');
if($retourLibrairie){
					echo "<table class='table table-sm table-hover'>\n";
					echo "<thead>\n";
					echo "<tr>\n";
					echo "<th scope='col'>Identifiant de la librairie</th>\n";
					echo "<th scope='col'>Nom de la librairie</th>\n";
					echo "<th scope='col'>Adresse de la librairie</th>\n";
					echo "<tr>\n";
					echo "</thead>\n";

while($donnee_librairie=mysqli_fetch_assoc($retourLibrairie)){
						echo "<tbody>\n";
						echo "<tr>\n";
						echo "<td>".$donnee_librairie["id_lib"]."</td>\n";
						echo "<td>".$donnee_librairie["nom"]."</td>\n";
						echo "<td>".$donnee_librairie["adresse"]."</td>\n";
						echo "<tr>\n";
						echo "</tbody>\n"; 
			}
			echo "</table>\n";
		...
		<div class="col-lg-8 pagination pagination-lg" align="center">
			...
			echo "<li class='page-item'><a class='page-link badge badge-info' href='librairies.php'>Précédent</a></li>";
			for($i=1; $i<=$nombreDePages; $i++){
			     if($i==$pageActuelle){
			  		echo "<li class='page-item'><a class='page-link badge badge-info' href='librairies.php?page=$i''>$i</a></li>";
			     }else{
			          echo "<li class='page-item'> <a class='page-link badge badge-info' href='librairies.php?page=$i'>$i</a></li> ";
			     }
			}
			echo "<li class='page-item'><a class='page-link badge badge-info' href='librairies.php?page=$i+1'>Suivant</a></li>";
				?>
		</div>
		</div>
	</div>
