...
//Maintenant occupons nous de l'ajout des bibiothéquaires
if (isset($_POST['ajouter'])) {
    $prenom=htmlspecialchars($_POST['prenom']);
    $nom=htmlspecialchars($_POST['nom']);
    $login=htmlspecialchars($_POST['login']);
    $password=($_POST['password']);
  if (!empty($_POST['prenom']) AND !empty($_POST['nom']) AND !empty($_POST['login']) AND !empty($_POST['password'])) {
      $selection = "SELECT * FROM utilisateurs WHERE password='$password' || login='$login'";
      $results = mysqli_query($db, $selection);
      if (mysqli_num_rows($results) == 0) {
      $requette="INSERT INTO utilisateurs (prenom,nom,login,password) 
            VALUES('$prenom','$nom','$login','$password')";
      $Inscription = mysqli_query($db, $requette);
      $erreur_succes="bibiothéquaire ajouté(e) avec succés!";

      }else {
        $erreur="Login et mot de pass déja utilisés!";
      }
...
//Modification du bibiothéquaire
  if(isset($_POST['modifier'])){
    if(!empty($_POST['id_user']) && !empty($_POST['prenom']) && !empty($_POST['nom']) && !empty($_POST['login']) && !empty($_POST['password'])){
        $id_user=$_POST['id_user'];
        $prenom=$_POST['prenom'];
        $nom=$_POST['nom'];
        $login=$_POST['login'];
        $password=($_POST['password']);
        $requette="UPDATE utilisateurs SET prenom='$prenom', nom='$nom',login='$login',password='$password' WHERE id_user=$id_user";
    ...
    //Suppression du bibiothéquaire
      ...
      if(!empty($_POST['id_user'])){
        ...
        $erreur="Aucun(e) bibliothéquaire séléctionné(e)!";
...
<html lang="en">
  ...
  <head>
    ...
    <title>Ajout bibliothéquaire</title>
    <!-- Bootstrap CSS-->
  ...
  <div class="col-xl-10 col-sm-6 mb-3">
    <div class="row ajout">
      <div align="center" class="col-lg-12 header"> 
        <h1 align="center" class="section-heading text-uppercase"><i class="fas fa-2x fa-user-tie"></i></h1>
      </div>
      <div class="col-lg-12 content">
        <form action="ajout_bibliothequaire.php" method="POST">
          <table align="center">
          <tr>
          <td>
          <input class="form-control input" type="text" name="id_user" placeholder="Identifiant *">
          </td>
          </tr>
          <tr>
          <td>
          ...
          <tr>
          <td>
          <input class="form-control input" type="text" name="password" placeholder="Mot de pass *" value="<?php if(isset($password)) {echo $password;}  ?>">
          </td>
          </tr>
          <tr>
          <td align="center">
