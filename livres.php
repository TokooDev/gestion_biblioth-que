  ...
  <body id="page-top">
	...
	<div class="row">
		<div class="col-xl-12 col-sm-6 mb-3">
			<div class="col-lg-12 header"><h1 class="section-heading text-uppercase" align="center"><i class="fas fa-book"></i> Les livres en stock</h1></div>
			<div class="col-xl-12 col-sm-6 mb-3 content">
				<?php 
				$livreParpage=5; //Nous allons afficher 5 livre par page. 
//Une connexion SQL doit être ouverte avant cette ligne...
$retour_total=mysqli_query($db,'SELECT COUNT(*) AS total FROM livres');//rons le contenu de la requête dans $retour_total
$donnees_total=mysqli_fetch_assoc($retour_total); //On range retour sous la forme d'un tableau.
$total=$donnees_total['total']; //On récupère le total pour le placer dans la variable $total.
 
//Nous allons maintenant compter le nombre de pages.
$nombreDePages=ceil($total/$livreParpage);
...
$premiereEntree=($pageActuelle-1)*$livreParpage; // On calcul la première entrée à lire
 
// La requête sql pour récupérer les livre de la page actuelle.
$retourLivre=mysqli_query($db,'SELECT DISTINCT livres.id_livre,livres.titre,livres.auteurs,livres.maison_edition,librairie.nom,livres.prix
                                 FROM livres,librairie
                                 WHERE (livres.id_lib=librairie.id_lib)
                                 ORDER BY id_livre LIMIT '.$premiereEntree.', '.$livreParpage.'');
if($retourLivre){
					...
					echo "<th scope='col'>Maison d'édition</th>\n";
					echo "<th scope='col'>Librairie</th>\n";
					echo "<th scope='col'>Prix (F cfa)</th>\n";
					echo "<tr>\n";
					echo "</thead>\n";
while($donnee_livre=mysqli_fetch_assoc($retourLivre)){
						echo "<tbody>\n";
						echo "<tr>\n";
						echo "<td>".$donnee_livre["id_livre"]."</td>\n";
						echo "<td>".$donnee_livre["titre"]."</td>\n";
						echo "<td>".$donnee_livre["auteurs"]."</td>\n";
						echo "<td>".$donnee_livre["maison_edition"]."</td>\n";
						echo "<td>".$donnee_livre["nom"]."</td>\n";
						echo "<td>".$donnee_livre["prix"]."</td>\n";
						echo "<tr>\n";
						echo "</tbody>\n"; 
			}
			echo "</table>\n";
		...
		<div class="col-lg-8 pagination pagination-lg" align="center">
			...
			echo "<li class='page-item'><a class='page-link badge badge-info' href='livres.php'>Précédent</a></li>";
			for($i=1; $i<=$nombreDePages; $i++){
			     if($i==$pageActuelle){
			  		echo "<li class='page-item'><a class='page-link badge badge-info' href='livres.php?page=$i''>$i</a></li>";
			     }else{
			          echo "<li class='page-item'> <a class='page-link badge badge-info' href='livres.php?page=$i'>$i</a></li> ";
			     }
			}
			echo "<li class='page-item'><a class='page-link badge badge-info' href='livres.php?page=$i+1'>Suivant</a></li>";
				?>
		</div>
		</div>
		</div>
