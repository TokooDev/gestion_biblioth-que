  ...
  <body id="page-top">
	...
	<div class="row">
		<div class="col-xl-12 col-sm-6 mb-3">
			<div class="col-lg-12 header"><h1 align="center" class="section-heading text-uppercase"><i class="fas fa-archive"></i> Archives d'emprunts</h1>
			</div>
			<div class="col-xl-12 col-sm-6 mb-3 content">
			<?php 
				$archives=5; //Nous allons afficher 5 archives par page.
$retour_total=mysqli_query($db,'SELECT COUNT(*) AS total FROM archives'); //Nous récupérons le contenu de la requête dans $retour_total
$donnees_total=mysqli_fetch_assoc($retour_total); //On range retour sous la forme d'un tableau.
$total=$donnees_total['total']; //On récupère le total pour le placer dans la variable $total.
//Nous allons maintenant compter le nombre de pages.
$nombreDePages=ceil($total/$archives);
...
$premiereEntree=($pageActuelle-1)*$archives; // On calcul la première entrée à lire
 
// La requête sql pour récupérer les archives de la page actuelle.
$retourArchive=mysqli_query($db,'SELECT DISTINCT archives.id_arch,lecteurs.prenom,lecteurs.nom,livres.titre,archives.date_empt,archives.date_ret
                                 FROM archives,lecteurs,livres
                                 WHERE (archives.id_lecteur=lecteurs.id_lecteur AND archives.id_livre=livres.id_livre)
                                 ORDER BY id_arch LIMIT '.$premiereEntree.', '.$archives.'');
if($retourArchive){
          ...
          echo "<tr>\n";
          echo "</thead>\n";
while($donnees_archive=mysqli_fetch_assoc($retourArchive)){
            echo "<tbody>\n";
            echo "<tr>\n";
            echo "<td>".$donnees_archive["id_arch"]."</td>\n";
            echo "<td>".$donnees_archive["prenom"].' '.$donnees_archive["nom"]."</td>\n";
            echo "<td>".$donnees_archive["titre"]."</td>\n";
            echo "<td>".$donnees_archive["date_empt"]."</td>\n";
            echo "<td>".$donnees_archive["date_ret"]."</td>\n";
            echo "<tr>\n";
            echo "</tbody>\n"; 
      }
      echo "</table>\n";
		...
		<div class="col-xl-12 col-sm-6 mb-3 pagination pagination-lg" align="center">
			...
			echo "<li class='page-item'><a class='page-link badge badge-info' href='archives.php'>Précedent</a></li>";
			for($i=1; $i<=$nombreDePages; $i++){
			     if($i==$pageActuelle){
			  		echo "<li class='page-item'><a class='page-link badge badge-info' href='archives.php?page=$i''>$i</a></li>";
			     }else{
			          echo "<li class='page-item'> <a class='page-link badge badge-info' href='archives.php?page=$i'>$i</a></li> ";
			     }
			}
			echo "<li class='page-item'><a class='page-link badge badge-info' href='archives.php?page=$i+1'>Suivant</a></li>";
				?>
		</div>
		</div>
	</div> 
